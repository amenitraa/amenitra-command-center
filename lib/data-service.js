...
function mergeRelationships({ accounts, stakeholders, signals, painPoints, reminders, channels, plays, developments, contentAssets, contentAssetChannels, goals, tasks, launches, leadershipPrompts, automationSources, integrationSettings, strategyOverview, strategyAccountMoves, strategyChannelMoves, strategyKnowledgeGaps, marketWatchItems }) {
  const watchByScope = marketWatchItems.reduce((groups, item) => {
    const key = `${item.scope_type}:${item.scope_slug}:${item.item_type}`;
  ...
  const mergedAccounts = accounts.map((account) => ({
    ...
    painHierarchy: (watchByScope[`account:${account.slug}:pain`] || []).length
      ? watchByScope[`account:${account.slug}:pain`]
    ...
    signals: (watchByScope[`account:${account.slug}:signal`] || []).length
      ? watchByScope[`account:${account.slug}:signal`]
          .sort((a, b) => new Date(b.published_at || 0) - new Date(a.published_at || 0))
          .slice(0, 5)
          .map((item) => ({
            title: item.title,
  ...
  const mergedIndustryWarRooms = clone(sampleData.industryWarRooms).map((room) => ({
    ...room,
    signals: (watchByScope[`industry:${room.id}:signal`] || []).length
      ? watchByScope[`industry:${room.id}:signal`]
          .sort((a, b) => new Date(b.published_at || 0) - new Date(a.published_at || 0))
          .slice(0, 5)
          .map((item) => ({
            title: item.title,
      ...
      : room.signals,
    painHierarchy: (watchByScope[`industry:${room.id}:pain`] || []).length
      ? watchByScope[`industry:${room.id}:pain`]
          .sort((a, b) => new Date(b.published_at || 0) - new Date(a.published_at || 0))
          .slice(0, 5)
          .map((item) => ({
            issue: item.title,
  ...
  return {
    ...
    industryWarRooms: mergedIndustryWarRooms.length ? mergedIndustryWarRooms : clone(sampleData.industryWarRooms),
    ...
    strategyLab:
      strategyOverview.length || strategyAccountMoves.length || strategyChannelMoves.length || strategyKnowledgeGaps.length
        ? {
            overview: strategyOverview.length
              ? strategyOverview
                  .sort((a, b) => a.display_order - b.display_order)
                  .map((item) => ({ title: item.title, text: item.text }))
              : clone(sampleData.strategyLab.overview),
            accountMoves: strategyAccountMoves.length
              ? strategyAccountMoves.map((item) => ({
                  accountId: item.account_slug,
                  shift: item.strategic_shift,
                  nextMove: item.next_move
                }))
              : clone(sampleData.strategyLab.accountMoves),
            channelMoves: strategyChannelMoves.length
              ? strategyChannelMoves.map((item) => ({
                  channel: item.channel_name,
                  recommendation: item.recommendation
                }))
              : clone(sampleData.strategyLab.channelMoves),
            knowledgeGaps: strategyKnowledgeGaps.length
              ? strategyKnowledgeGaps
                  .sort((a, b) => a.display_order - b.display_order)
                  .map((item) => item.note)
              : clone(sampleData.strategyLab.knowledgeGaps)
          }
        : clone(sampleData.strategyLab),
    goals: goals.length
    ...
    integrationSettings: integrationSettings.length
      ? integrationSettings.map((item) => ({
          name: item.integration_name,
          status: item.status || "",
          summary: item.config_summary || "",
          lastSyncedAt: item.last_synced_at || ""
        }))
      : clone(sampleData.integrationSettings || [])
  };
}
...
async function loadSupabaseState() {
  ...
  try {
    const [
      ...
      strategyOverviewResult,
      strategyAccountMovesResult,
      strategyChannelMovesResult,
      strategyKnowledgeGapsResult,
      marketWatchItemsResult
    ] = await Promise.all([
      client.from("industries").select("id, name"),
      client.from("accounts").select("id, slug, name, sub_industry, revenue_target, status, focus, market_position, consultant_positioning, industry_id"),
      ...
      client.from("account_pain_points").select("rank, note, account_id"),
      ...
      client.from("tasks").select("id, title, due_label, priority, source, status, assigned_to, team_function, task_link, account_slug"),
      client.from("launches").select("id, title, phase, owner, note, due_date, account_slug, progress_percent"),
      client.from("leadership_prompts").select("title, detail"),
      client.from("automation_sources").select("source_name, status, note"),
      client.from("integration_settings").select("integration_name, status, config_summary, last_synced_at"),
      client.from("strategy_overview_cards").select("title, text, display_order"),
      client.from("strategy_account_moves").select("strategic_shift, next_move, account_id"),
      client.from("strategy_channel_moves").select("channel_name, recommendation"),
      client.from("strategy_knowledge_gaps").select("note, display_order"),
      client.from("market_watch_items").select("scope_type, scope_slug, item_type, title, summary, why_it_matters, response, source_label, source_url, published_at")
    ]);
    ...
    const errors = [
      ...
      strategyOverviewResult.error,
      strategyAccountMovesResult.error,
      strategyChannelMovesResult.error,
      strategyKnowledgeGapsResult.error,
      marketWatchItemsResult.error
    ].filter(Boolean);
    ...
    const mergedState = mergeRelationships({
      ...
      painPoints: (painPointsResult.data || []).map((item) => ({
        ...item,
        account_slug: accountSlugById[item.account_id]
      })),
      ...
      integrationSettings: integrationSettingsResult.data || [],
      strategyOverview: strategyOverviewResult.data || [],
      strategyAccountMoves: (strategyAccountMovesResult.data || []).map((item) => ({
        ...item,
        account_slug: accountSlugById[item.account_id]
      })),
      strategyChannelMoves: strategyChannelMovesResult.data || [],
      strategyKnowledgeGaps: strategyKnowledgeGapsResult.data || [],
      marketWatchItems: marketWatchItemsResult.data || []
    });
...
async function updateAccountInSupabase(client, account) {
  ...
  const painPointsDelete = await client.from("account_pain_points").delete().eq("account_id", accountId);
  const signalsDelete = await client.from("account_signals").delete().eq("account_id", accountId);

  if (stakeholdersDelete.error || remindersDelete.error || painPointsDelete.error || signalsDelete.error) {
    throw new Error(
      [
        stakeholdersDelete.error?.message,
        remindersDelete.error?.message,
        painPointsDelete.error?.message,
        signalsDelete.error?.message
      ]
        .filter(Boolean)
        .join(" | ")
    );
  ...
  if (account.painHierarchy.length) {
    const painPointsInsert = await client.from("account_pain_points").insert(
      account.painHierarchy.map((note, index) => ({
        account_id: accountId,
        rank: index + 1,
        note
      }))
    ...
    if (painPointsInsert.error) {
      throw new Error(painPointsInsert.error.message);
    }
  }

  if (account.signals.length) {
    const signalsInsert = await client.from("account_signals").insert(
    ...
    if (signalsInsert.error) {
      throw new Error(signalsInsert.error.message);
    }
  }
}
